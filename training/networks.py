...
@persistence.persistent_class
class EDMPrecond_c_skip(torch.nn.Module, PyTorchModelHubMixin):
    def __init__(self,
        img_resolution,                     # Image resolution.
        img_channels,                       # Number of color channels.
        label_dim       = 0,                # Number of class labels, 0 = unconditional.
        ...
        **model_kwargs,                     # Keyword arguments for the underlying model.
        ...
        self.sigma_skip = sigma_skip if sigma_skip is not None else sigma_data
        self.model = globals()[model_type](img_resolution=img_resolution, in_channels=img_channels, out_channels=img_channels, label_dim=label_dim, **model_kwargs)
    ...
    def forward(self, x : torch.Tensor, sigma : torch.Tensor, class_labels=None, force_fp32=False, **model_kwargs):
        ...
        if self.sigma_skip == float('inf'):
            c_skip = 1
        else:
            c_skip = self.sigma_skip ** 2 / (sigma ** 2 + self.sigma_skip ** 2)

        c_out = (((1 - c_skip) * self.sigma_data)**2 + (c_skip * sigma)**2).sqrt()
